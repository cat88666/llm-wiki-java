...
- **更新** `concepts/08-distributed/概念-可观测性.md`
  - 合并：诊断工具速查与 Arthas 原理
  - 合并：Metrics → Tracing → Logging → 现场工具 的通用排障闭环
  - 合并：CPU 飙高、JIT 预热、FullGC / OOM、慢 SQL、数据库死锁 / 连接池、MQ 堆积、基础设施故障案例
  - 合并：线上排障规律总结
- **删除** `synthesis/设计-线上问题排查.md`
- **更新** `synthesis/设计-容量规划与性能基准.md`
  - 将对线上问题排查专题页的依赖改为 `概念-可观测性`
- **更新** `wiki/index.md`
  - 删除线上问题排查重复专题页条目

## [2026-05-13] Merge | 大厂秒杀实践收口到设计-秒杀系统，删除 1 个重复专题页

- 触发：同一主题下存在“秒杀系统主稿”与“大厂秒杀实践补充页”两份重叠专题
- **更新** `synthesis/设计-秒杀系统.md`
  - 合并：为什么大厂不全靠 Redis
  - 合并：阿里 Inventory Hint（热点 SQL 分组、Row Cache、组提交）
  - 合并：小红书合并秒杀（Leader-Follower 合并提交）
  - 合并：秒杀方案选型表
- **删除** `synthesis/设计-大厂秒杀实践.md`
- **更新** `summaries/主题-三高体系.md`
  - 将大厂秒杀实践引用统一改为 `设计-秒杀系统`
- **更新** `synthesis/设计-彩票系统核心设计.md`
  - 将关联页统一改为 `设计-秒杀系统`
- **更新** `synthesis/设计-P8高频边界问题.md`
  - 将秒杀追问补充页统一改为 `设计-秒杀系统`
- **更新** `wiki/index.md`
  - 删除大厂秒杀实践重复专题页条目

## [2026-05-13] Merge | 消息队列场景综合收口到机制-消息队列可靠性，删除 1 个重复专题页

- 触发：继续按“单主题单归宿”原则收敛知识碎片
- **更新** `concepts/08-distributed/机制-消息队列可靠性.md`
  - 合并：Spring Event vs MQ
  - 合并：BlockingQueue vs MQ
  - 合并：拉模式 vs 推模式
  - 合并：消息乱序解法
  - 合并：Kafka 单分区提吞吐
  - 合并：为什么不建议用 MQ 做订单超时关闭
  - 合并：本地消息表与消息表 SQL 设计
- **删除** `synthesis/设计-消息队列场景综合.md`
- **更新** `wiki/index.md`
  - 删除消息队列场景综合重复专题页条目

## [2026-05-13] Merge | JVM调优实战收口到机制-GC算法与垃圾收集器，删除 1 个重复专题页

- 触发：继续按“单主题单归宿”原则收敛知识碎片
- **更新** `concepts/03-jvm/机制-GC算法与垃圾收集器.md`
  - 合并：调优方法论、GC 基线、常用调优工具
  - 合并：FullGC / OOM / YoungGC / STW 典型排查
  - 合并：Java 8 → 11 默认 GC 差异
  - 合并：常用 JVM 参数速查
- **删除** `synthesis/设计-JVM调优实战.md`
- **更新** `synthesis/设计-容量规划与性能基准.md`
  - 将对 JVM 调优专题页的依赖改为 `机制-GC算法与垃圾收集器`
- **更新** `wiki/index.md`
  - 删除 JVM 调优重复专题页条目

## [2026-05-13] Merge | 云原生选型收口到机制-容器化与Docker，删除 1 个重复专题页

- 触发：继续按“单主题单归宿”原则收敛知识碎片
- **更新** `concepts/08-distributed/机制-容器化与Docker.md`
  - 合并：IaaS / PaaS / SaaS / Serverless 交付模型
  - 合并：公有云 / 私有云 / 混合云
  - 合并：Java 冷启动与 GraalVM / Quarkus / Keep Warm
  - 合并：无状态设计原则
  - 合并：Java 后端云原生改造要点
- **删除** `synthesis/设计-云原生选型.md`
- **更新** `wiki/index.md`
  - 删除云原生选型重复专题页条目

## [2026-05-13] Merge | MySQL 大表与查询优化收口到概念-SQL查询优化，删除 1 个重复专题页

- 触发：继续按“单主题单归宿”原则收敛知识碎片
- **更新** `concepts/06-storage/概念-SQL查询优化.md`
  - 合并大表场景：2000w 阈值、深分页、后缀查询冗余字段、扫表任务跳页
  - 合并运维场景：Online DDL / pt-osc / gh-ost
  - 合并数据清理：分批按主键删除
...
## [2026-05-13] Merge | DDD 主题收口到概念-DDD，删除 1 个重复专题页

- 触发：用户要求同一主题集中归档，减少知识碎片；明确指定将 `synthesis/设计-DDD落地实战.md` 并入 `concepts/09-practice/概念-DDD.md`
- **更新** `concepts/09-practice/概念-DDD.md`
  - 新增：限界上下文 → 微服务拆分落地流程
  - 新增：上下文映射关系表（Shared Kernel / Customer-Supplier / ACL / OHS）
  - 新增：聚合根粒度实战判断与订单聚合代码示例
  - 新增：四层架构落地视角与应用层/领域层职责区分
  - 新增：领域事件实战代码（同进程 Spring Event）
  - 新增：CQRS 典型落地与适用边界
  - 新增：贫血模型 / ORM 污染领域层等反模式示例
  - 新增：聚合根粒度面试话术
- **删除** `synthesis/设计-DDD落地实战.md`
- DDD 主题后续统一收口到 `概念-DDD.md`

## [2026-05-13] Merge | 支付专题收口到 mock-interview-eson，删除 2 个重叠页

- 触发：用户要求减少知识库碎片，在不丢内容的前提下合并并删除部分文件
- **更新** `synthesis/mock-interview-eson.md`
  - 新增 frontmatter，补充 aliases/related/sources，提升为支付专题集中入口
  - 新增「2.A 支付中台速查补充」章节，收口以下内容：
    - 支付中台整体架构（交易核心/路由核心/渠道核心/账务核心）
    - 渠道路由维度（金额/成本/成功率/商户配置/地域限额）
    - 余额三态 + 在途户
    - 充值/提现链路最小闭环
    - 双记账 / append-only 流水原则
    - 高并发优化与资损防控总纲
- **删除** `synthesis/设计-支付系统设计.md`
- **删除** `synthesis/设计-账户钱包系统.md`
- **更新引用**
  - `synthesis/设计-彩票系统核心设计.md`
  - `synthesis/设计-项目难点表达.md`
  - `concepts/08-distributed/机制-RocketMQ.md`
  - `concepts/08-distributed/机制-数据加密与脱敏.md`
- **更新** `wiki/index.md`
  - 删除 2 个支付碎片页条目
  - 强化 `模拟面试-Eson` 作为支付专题入口的描述
