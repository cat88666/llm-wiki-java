...
秒杀的核心矛盾是瞬时极高并发和有限库存。工程目标不是让数据库抗住所有请求，而是用前端、网关、缓存、消息队列和数据库构成漏斗，把压力逐层削掉。
...
Redis Lua 负责拦截大部分并发，数据库仍必须做最终防超卖：
...
Redis 扣减成功但 MQ 或 DB 写入失败时，需要通过本地消息表、补偿任务或对账任务修正库存状态，避免 Redis 和 DB 长期不一致。

### 热点行与库存拆分

单个商品库存行会成为 InnoDB 行锁热点。常见优化是把一个库存拆成多个 bucket，扣减时随机命中一个 bucket，查询时聚合总量。

| 方案 | 优点 | 代价 |
|------|------|------|
| Redis 预扣 + MQ | 吞吐高，保护 DB | 短暂不一致，需要补偿 |
| DB 乐观锁 | 强一致，简单 | 热点行竞争严重 |
| 库存分桶 | 降低行锁冲突 | 聚合查询和兜底逻辑复杂 |
| 队列串行化 | 不超卖，逻辑简单 | 吞吐受单队列限制 |
...
阿里 Inventory Hint、小红书合并秒杀，本质都是把大量热点行 `UPDATE` 请求合并成少量批次提交，减少行锁等待、索引遍历和事务提交次数。适合强一致库存、数据库能力较强的场景；普通业务优先使用 Redis 预扣 + MQ 削峰 + DB 兜底。
