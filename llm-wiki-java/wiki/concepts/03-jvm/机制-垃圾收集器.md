...
| 章节 | 内容 |
|------|------|
| [一、第一性原理](#一第一性原理) | GC 为何存在、根本矛盾 |
| [二、核心机制](#二核心机制) | 可达性分析、三种算法、三色标记、四种引用、GC 触发条件 |
| [三、Java 核心使用](#三java-核心使用) | 收集器选型表、常用 JVM 参数速查 |
| [四、核心使用原则](#四核心使用原则) | 调优方法论五步、正常基线 |
| [五、使用案例](#五使用案例) | 频繁 FullGC / OOM / YGC 过频 / STW 过长排查 |
| [六、综合对比](#六综合对比) | 五组关键权衡 |
| [七、生产风险](#七生产风险) | OOM 五种类型、ThreadLocal 泄漏、调优工具 |
| [八、与其他概念的关系](#八与其他概念的关系) | JVM 内存模型、ThreadLocal、对象池 |
| [九、应用边界](#九应用边界) | 收集器选型决策、调优口诀 |
...
核心原则：**先修代码问题，再调参数**。参数只能缓解，内存泄漏和对象 churn 往往要靠代码修复。
...
**G1 调优**：通过 `-XX:MaxGCPauseMillis=200` 设定目标停顿时间，G1 会自动调整 Region 数量。
...
| 权衡维度 | 一方 | 另一方 | 判断要点 |
|---------|------|--------|---------|
| STW vs 并发 | 全量暂停回收，实现简单 | 并发 GC 复杂，但停顿更短 | 对延迟敏感选并发 |
| 吞吐量 vs 延迟 | Parallel 追吞吐 | CMS/G1/ZGC 追延迟 | 批处理选吞吐，在线服务选延迟 |
| 碎片 vs 停顿 | CMS 快但有碎片 | G1/ZGC 整理开销更大但无碎片 | 长期运行选无碎片 |
| G1 vs ZGC | G1 通用稳定 | ZGC 适合超低延迟 | 堆 < 8G 选 G1，> 8G 且延迟敏感选 ZGC |
| 参数调优 vs 代码优化 | 参数只能缓解 | 内存泄漏要靠代码修复 | 先修代码，再调参数 |
