...
| GC Root 类型 | 示例 |
|--------------|------|
| 虚拟机栈引用 | 当前方法栈帧中的局部变量、方法参数、临时变量引用的对象 |
...
| 场景 | 排查路径 |
|------|----------|
| 系统仍在运行 | 先用 `jstat` 看 YoungGC/FullGC 频率与各区使用；再用 `jmap` 看对象分布或导出 dump；用 `jstack` 看阻塞、死锁、高 CPU 线程；最后结合 VisualVM/MAT/Arthas 定位代码 |
| 已发生 OOM | 依赖 `-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=...` 自动保留现场；用 MAT/VisualVM 找 retained size 最大对象、异常 GC Root 引用链和相关线程，再回到代码分析泄漏或大对象来源 |
| CPU 同时很高 | `top -Hp <pid>` 找最耗 CPU 的线程；线程 id 转 16 进制后在 `jstack` 中定位具体栈帧；优化热点方法，减少无效循环、锁竞争或对象创建 |
...
如果 FullGC 频繁但没有 OOM，通常说明每次 FullGC 还能回收不少对象。优先判断是否有“存活时间不长但对象较大/数量较多”的对象提前进入老年代：可尝试增大新生代或 Survivor 空间，让短命对象在 YoungGC 阶段被回收；若调整后 FullGC 明显减少，说明晋升过快是主要原因。
...
生产建议开启：`-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/data/dump`。dump 文件只是一张堆快照，分析时重点看两件事：哪些对象 retained size 最大；这些对象为什么被 GC Root 引用而无法回收。
