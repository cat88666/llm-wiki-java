...
| 标题索引 | 概述 |
| --- | --- |
| [一、第一性原理](#一第一性原理) | 不同数据生命周期不同、分区管理、分代假说 |
| [二、核心机制](#二核心机制) | 五大区域、堆分代、对象晋升、对象创建流程、TLAB、方法区演变、堆外内存 |
| [三、Java 核心使用](#三java-核心使用) | JVM 内存参数、内存查看命令 |
| [四、核心使用原则](#四核心使用原则) | Xms=Xmx、MaxMetaspaceSize、新生代比例、堆外监控 |
| [五、使用案例](#五使用案例) | Spring Boot 配置示例、OOM 排查定位 |
| [六、综合对比](#六综合对比) | 堆 vs 栈、永久代 vs 元空间、分配方式对比 |
| [七、生产风险](#七生产风险) | 堆 OOM、元空间 OOM、栈溢出、直接内存、NMT |
| [八、与其他概念的关系](#八与其他概念的关系) | GC、类加载、JIT、NIO |
| [九、应用边界](#九应用边界) | JVM 内存模型 ≠ JMM、堆外内存边界 |
...
| OOM 类型 | 排查路径 |
|---------|---------|
| `OutOfMemoryError: Java heap space` | 堆内对象过多 → `jmap -dump` 导出堆转储 → MAT / VisualVM 分析大对象和 GC Root 引用链 |
| `OutOfMemoryError: Metaspace` | 元空间满 → 排查类加载器泄漏（动态代理、JSP 热加载、CGLIB 大量生成类） |
| `StackOverflowError` | 递归过深 → 检查递归终止条件，必要时改为迭代或调大 `-Xss` |
