...
| 标题索引 | 概述 |
| --- | --- |
| [一、用线程隔离替代同步](#一用线程隔离替代同步) | 隔离代替同步、不共享就无竞争 |
| [二、ThreadLocalMap 存储结构](#二threadlocalmap-存储结构) | ThreadLocalMap 结构、弱引用key/强引用value、读写流程 |
| [三、线程池内存泄漏链路](#三线程池内存泄漏链路) | 两条引用链、普通线程 vs 线程池、remove() |
| [四、父子线程上下文传递](#四父子线程上下文传递) | InheritableThreadLocal、TTL 原理 |
| [五、ThreadLocal 典型场景](#五threadlocal-典型场景) | 请求上下文、事务管理、链路追踪 |
| [六、线程隔离方案对比](#六线程隔离方案对比) | ThreadLocal vs synchronized vs ScopedValue |
| [七、ThreadLocal 生产风险](#七threadlocal-生产风险) | 线程池污染、大对象泄漏、TTL 丢失 |
| [八、ThreadLocal 的并发位置](#八threadlocal-的并发位置) | 线程池、JMM、AQS |
| [九、ThreadLocal 适用边界](#九threadlocal-适用边界) | 适合/不适合场景、JDK 21+ 替代方案 |
