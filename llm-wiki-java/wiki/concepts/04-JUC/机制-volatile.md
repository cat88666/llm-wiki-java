...
| 标题索引 | 概述 |
| --- | --- |
| [一、轻量级可见性为什么存在](#一轻量级可见性为什么存在) | 为何需要 volatile、与 synchronized 的本质区别 |
| [二、可见性、屏障与非原子性](#二可见性屏障与非原子性) | LOCK 前缀、MESI、4 种内存屏障、i++ 误区 |
| [三、volatile 典型正确用法](#三volatile-典型正确用法) | DCL 双重检验锁、状态标志、发布对象 |
| [四、volatile 与锁和原子类对比](#四volatile-与锁和原子类对比) | volatile vs synchronized vs AtomicXxx |
| [五、volatile 生产误用风险](#五volatile-生产误用风险) | i++ 误用、多变量原子性、过度依赖 volatile |
| [六、volatile 的并发语义来源](#六volatile-的并发语义来源) | JMM、synchronized、CAS |
| [七、volatile 适用边界](#七volatile-适用边界) | 适合/不适合场景 |
