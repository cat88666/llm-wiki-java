...
| 标题索引 | 概述 |
| --- | --- |
| [一、内置互斥锁解决什么](#一内置互斥锁解决什么) | 互斥保证三性、Monitor 抽象 |
| [二、锁对象、对象头与 Monitor](#二锁对象对象头与-monitor) | 用法与锁对象、Mark Word、ObjectMonitor、三性保证 |
| [三、偏向、轻量与重量级锁](#三偏向轻量与重量级锁) | 无锁→偏向→轻量→重量、各阶段原理、JDK 15 废弃偏向锁 |
| [四、JIT 对锁的优化](#四jit-对锁的优化) | 锁消除、锁粗化 |
| [五、synchronized 与 ReentrantLock 选型](#五synchronized-与-reentrantlock-选型) | synchronized vs ReentrantLock 6维度 |
| [六、Monitor 生产误用风险](#六monitor-生产误用风险) | 死锁、锁对象误用、wait/notify 注意事项 |
| [七、synchronized 的底层连接](#七synchronized-的底层连接) | JMM、CAS、AQS |
| [八、synchronized 适用边界](#八synchronized-适用边界) | 何时用 synchronized / ReentrantLock |
...
## 五、synchronized 与 ReentrantLock 选型
