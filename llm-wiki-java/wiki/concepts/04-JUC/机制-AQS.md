...
| 标题索引 | 概述 |
| --- | --- |
| [一、同步器为什么需要骨架](#一同步器为什么需要骨架) | 同步状态、等待队列、tryAcquire 模板方法 |
| [二、state 与同步队列](#二state-与同步队列) | volatile state、CLH 双向队列、入队/阻塞/唤醒流程 |
| [三、Condition 条件队列](#三condition-条件队列) | await/signal、条件队列与同步队列的节点流转 |
| [四、独占模式与共享模式](#四独占模式与共享模式) | ReentrantLock vs CountDownLatch/Semaphore |
| [五、AQS 工具类实现语义](#五aqs-工具类实现语义) | ReentrantLock/CountDownLatch/Semaphore/CyclicBarrier 原理 |
| [六、AQS 选型对比](#六aqs-选型对比) | synchronized vs ReentrantLock、公平锁 vs 非公平锁 |
| [七、AQS 的底层依赖](#七aqs-的底层依赖) | CAS、volatile、synchronized |
| [八、AQS 自定义边界](#八aqs-自定义边界) | 何时自定义同步器、何时用现成工具 |
