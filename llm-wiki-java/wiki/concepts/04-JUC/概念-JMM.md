...
| 标题索引 | 概述 |
| --- | --- |
| [一、JMM 统一的并发语义](#一jmm-统一的并发语义) | 三大并发问题、JMM 存在理由、JMM vs JVM内存结构 |
| [二、主内存、happens-before 与屏障](#二主内存happens-before-与屏障) | 主内存/工作内存、happens-before 8条规则、内存屏障 |
| [三、硬件缓存与屏障真相](#三硬件缓存与屏障真相) | Store Buffer、Invalidation Queue、MESI 不够用的原因 |
| [四、JMM 推理的典型用法](#四jmm-推理的典型用法) | DCL 双重检验锁、as-if-serial 对比 |
| [五、JMM 易混概念辨析](#五jmm-易混概念辨析) | JMM vs JVM 内存结构、JMM vs MESI |
| [六、JMM 的关键字落地](#六jmm-的关键字落地) | volatile、synchronized、CAS |
| [七、JMM 使用边界](#七jmm-使用边界) | 何时需要理解 JMM、何时不需要直接操作 |
