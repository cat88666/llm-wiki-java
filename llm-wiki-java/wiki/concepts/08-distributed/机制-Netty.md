...
ServerBootstrap bootstrap = new ServerBootstrap()
    ...
    .childOption(ChannelOption.SO_KEEPALIVE, true)    // TCP 层心跳，检测 2 小时无数据的死连接
    .childOption(ChannelOption.SO_RCVBUF, 4 * 1024 * 1024)  // 接收缓冲 4MB（跨地域高延迟链路需大窗口）
...
| `SO_RCVBUF/SNDBUF` | 调大 TCP 窗口：高 RTT 链路（跨地域）需要大窗口才能打满带宽 | 4MB（跨地域），256KB（同机房） |
| `SO_KEEPALIVE` | 2h 无数据后 TCP 层发探测包，清除死连接 | 应配合 `IdleStateHandler` 应用层心跳 |
| `WriteBufferWaterMark` | 写缓冲超 high 水位关闭 `channel.isWritable()`；降到 low 水位重开 | 防止慢客户端导致内存溢出 |
| `SO_BACKLOG` | 内核半连接队列大小，高并发下调大防连接排队超时 | 1024-4096 |

**`TCP_NODELAY` 原理**：Nagle 算法将小包积累到一个 MSS（1460字节）才发送，适合 FTP 等大文件传输；对游戏对战、IM 消息（每包几十字节）会引入 200ms+ 的额外延迟。关闭 Nagle 后包立即发出，延迟降低 20-30%。

**写缓冲水位实践**：
```java
// Handler 中写数据前检查可写性（背压）
