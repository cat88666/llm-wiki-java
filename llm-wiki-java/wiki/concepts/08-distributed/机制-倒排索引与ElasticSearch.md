...
关系数据库的 B+树索引（见 [[机制-InnoDB索引]]）解决的是"按字段精确/范围查找记录"。但全文搜索需要反过来问：**哪些文档包含这个词？** 传统 B+树无法高效回答这个问题。倒排索引（Inverted Index）正是为此而生——它以词条为键、以文档集合为值，把"正向"的文档→词条关系，翻转为"倒向"的词条→文档关系。
...
- **对比 [[机制-InnoDB索引]]**：InnoDB B+树是"文档→词条"的正向索引，适合精确/范围查询；ES 倒排索引是"词条→文档"，适合全文搜索；两者解决不同问题，生产中往往 MySQL 存源数据 + ES 搜索
