...
name: "Seata框架机制"
layer: L7
aliases: ["Seata", "AT模式", "TCC模式", "分布式事务框架", "全局事务", "TC", "TM", "RM"]
related:
  - "[[概念-分布式系统理论]]"
  - "[[机制-Spring事务]]"
  - "[[概念-幂等设计]]"
...
适合多数据源场景（MySQL + Redis + ES 混合事务），性能极高（无锁）。
...
**AT vs XA 核心区别**：AT 一阶段直接提交本地事务（释放行锁），通过 UNDO_LOG 保证回滚能力，性能好但只有最终一致；XA 一阶段持有资源锁到二阶段决议，强一致但锁持有时间长、性能差。
...
- 基于 [[机制-Spring事务]] 扩展：`@GlobalTransactional` 是 `@Transactional` 的分布式版，底层仍走 Spring AOP 代理
- 幂等控制参考 [[概念-幂等设计]]：TCC 的分布式事务记录表是"一锁、二判、三更新"的工程实践
- 理论基础来自 [[概念-分布式系统理论]]：Seata 本质是 2PC 的工程化实现，AT 为最终一致（BASE），XA 为强一致（ACID）
