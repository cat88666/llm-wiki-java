...
name: "Kafka"
layer: L7
aliases: ["Kafka", "消息流平台", "Kafka重平衡", "ISR", "高水位", "Leader Epoch", "渐进式重平衡", "Kafka顺序消费", "Kafka存储结构", "KRaft", "acks", "HW", "LEO"]
tags: ["#distributed"]
related:
  - "[[机制-RabbitMQ]]"
  - "[[机制-Zookeeper]]"
...
| [四、消息可靠性：acks + ISR + HW](#四消息可靠性acks--isr--hw) | 生产者确认、ISR 同步、高水位 |
| [五、重平衡（Rebalance）](#五重平衡rebalance) | 触发条件、渐进式重平衡、减少重平衡手段 |
| [六、顺序消费与消费语义](#六顺序消费与消费语义) | 同 Partition 有序、三种消费语义 |
| [七、与其他 MQ 的对比](#七与其他-mq-的对比) | Kafka vs RocketMQ vs RabbitMQ |
| [八、生产者与消费者调优参数](#八生产者与消费者调优参数) | 关键参数与调优建议 |
| [九、大流量实战](#九大流量实战) | 消息积压治理、广播模式 |
| [十、关键权衡与应用边界](#十关键权衡与应用边界) | 高吞吐 vs 功能，适合 vs 不适合场景 |
...
与 Leader 保持同步的 Follower 集合。Follower 落后超过 `replica.lag.max.ms`（默认 10s）则被移出 ISR。
