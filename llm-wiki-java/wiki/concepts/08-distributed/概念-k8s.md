...
| 标题索引 | 概述 |
| --- | --- |
| [一、K8s 解决的容器编排问题](#一k8s-解决的容器编排问题) | K8s 解决多机容器编排，而不是单机容器运行 |
| [二、控制面与工作节点架构](#二控制面与工作节点架构) | 控制面、工作节点、etcd、API Server、Scheduler、Controller、kubelet |
| [三、Pod 与工作负载对象](#三pod-与工作负载对象) | Pod、Label、Deployment、ReplicaSet、DaemonSet、Job、Namespace |
...
| [十、K8s 工程取舍](#十k8s-工程取舍) | K8s 引入成本、Service 暴露方式、iptables vs IPVS、存储动态供给 |
| [十一、K8s 适用边界](#十一k8s-适用边界) | 适合场景与不适合场景 |
