...
- `OK(0)` / `CANCELLED(1)` / `DEADLINE_EXCEEDED(4)` / `NOT_FOUND(5)` / `UNAVAILABLE(14)`
- `RESOURCE_EXHAUSTED(8)` — 限流/配额超限时使用
...
- 与 [[机制-微服务与SpringCloud]] 配合：Spring Cloud 生态通过 `grpc-spring-boot-starter` 集成 gRPC，Nacos 作为注册中心。
- 优于 [[机制-消息队列可靠性]] 的场景：同步请求-响应（RPC），而非异步消息投递。

## 应用边界

**用 gRPC：**
- 内部微服务之间高频 RPC（Eson 简历中游戏服务跨地域通信，延迟降低 15%）
