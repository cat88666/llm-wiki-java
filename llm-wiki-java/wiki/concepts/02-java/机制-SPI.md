...
| 标题索引 | 概述 |
| --- | --- |
| [一、SPI 的本质与价值](#一spi-的本质与价值) | API 与 SPI 的控制反转 |
| [二、Java SPI 执行链路](#二java-spi-执行链路) | `META-INF/services` + ServiceLoader |
| [三、类加载器与委派破局](#三类加载器与委派破局) | TCCL 解决父加载器看不到子实现 |
| [四、主流框架 SPI 变体](#四主流框架-spi-变体) | Java SPI / Dubbo SPI / Spring SPI |
| [五、设计与实现原则](#五设计与实现原则) | 扩展点命名、兼容策略、默认实现 |
| [六、选型对比结论](#六选型对比结论) | 不同 SPI 机制适用场景 |
| [七、生产风险与排查](#七生产风险与排查) | 全量扫描、类冲突、实例线程安全 |
| [八、关系与边界](#八关系与边界) | 与代理、反射、配置中心关系 |
| [九、面试速答口径](#九面试速答口径) | 高频问题答案 |
