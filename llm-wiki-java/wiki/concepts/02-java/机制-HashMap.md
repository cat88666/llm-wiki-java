...
| 标题索引 | 概述 |
| --- | --- |
| [一、HashMap 本质](#一hashmap-本质) | hash 映射、冲突处理、O(1) 的代价 |
| [二、数据结构演进](#二数据结构演进) | JDK7 数组+链表 → JDK8 +红黑树、树化/退化阈值 |
| [三、Hash 计算与寻址](#三hash-计算与寻址) | 扰动函数、2^n 容量、& 寻址、tableSizeFor |
| [四、put / get 完整流程](#四put--get-完整流程) | put 七步、get 三步、hashCode/equals 契约 |
| [五、扩容机制](#五扩容机制) | 0.75 负载因子、触发条件、JDK8 高低位拆分 |
| [六、线程安全陷阱](#六线程安全陷阱) | JDK7 死循环机制、JDK8 数据覆盖与 size 不准 |
| [七、ConcurrentHashMap 演进](#七concurrenthashmap-演进) | JDK7 分段锁 → JDK8 CAS+节点锁、为何选 synchronized |
| [八、fail-fast vs fail-safe](#八fail-fast-vs-fail-safe) | modCount 检测、弱一致性迭代、COW 写时复制 |
| [九、对比与选型](#九对比与选型) | HashMap/CHM/Hashtable/COW 全量对比、选型决策树 |
| [十、生产风险](#十生产风险) | 可变 key、容量预估、null 二义性、大 Map 扩容卡顿 |
