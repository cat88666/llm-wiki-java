...
| 标题索引 | 概述 |
| --- | --- |
| [一、四大机制的存在理由与知识地图](#一四大机制的存在理由与知识地图) | 四大机制的存在理由、知识地图 |
...
| [六、MVCC vs 锁 / RC vs RR / 四种隔离级别 / 三日志横向](#六mvcc-vs-锁--rc-vs-rr--四种隔离级别--三日志横向) | MVCC vs 锁、RC vs RR 选型、四种隔离级别、三日志横向对比 |
| [七、生产风险](#七生产风险) | 长事务、无索引锁退化、死锁、刷盘策略、索引失效 |
| [八、MySQL 在存储体系的位置](#八mysql-在存储体系的位置) | B+树、CAS、分布式事务、SQL优化 |
| [九、各机制适用边界与常见误用](#九各机制适用边界与常见误用) | 各机制的适用边界与常见误用 |
...
## 六、MVCC vs 锁 / RC vs RR / 四种隔离级别 / 三日志横向
