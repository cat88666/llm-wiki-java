...
**序列化**：Hessian2（默认，跨语言，性能好）→ Java 原生序列化（慢，不推荐）→ fastjson2；扩展支持 Kryo、Fury（蚂蚁，号称比 Hessian2 快 100 倍）、Protobuf。
...
- **依赖 [[机制-ZAB协议与Zookeeper]]**：Dubbo 默认使用 ZooKeeper 作为注册中心（利用 ZK 临时节点做服务上下线感知，Watch 机制做变更通知）
