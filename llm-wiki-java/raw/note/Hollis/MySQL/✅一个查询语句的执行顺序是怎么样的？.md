...
> 原文：[https://www.yuque.com/hollis666/asgng6/ihsu18bwvr9csnzu](https://www.yuque.com/hollis666/asgng6/ihsu18bwvr9csnzu)
> 
...
1. **JOIN**：
- 根据employees.department_id = [departments.id](http://departments.id)条件执行JOIN操作，将两个表的数据合并。
1. **WHERE**：
- 应用WHERE过滤条件employees.salary > 50000，只保留薪水超过50,000的员工记录。
1. **GROUP BY**：
- [根据departments.name](http://根据departments.name)对结果集进行分组。
1. **HAVING**：
