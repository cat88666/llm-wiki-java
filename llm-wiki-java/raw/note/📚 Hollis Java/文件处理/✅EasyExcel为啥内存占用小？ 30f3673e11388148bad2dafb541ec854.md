...
> 原文：[https://www.yuque.com/hollis666/asgng6/lwaomovww7yr4v7z](https://www.yuque.com/hollis666/asgng6/lwaomovww7yr4v7z)
> 
...
EasyExcel 之所以能在处理大型 Excel 文件时保持较低的内存占用，主要归功于其**SAX解析设计（逐行读取+事件机制）**和**磁盘缓存策略**。（本文主要介绍EasyExcel的文件读取占用内存小的原因，文件写入其实和POI的 SXSSFWorkbook  原理一样的。直接看[https://www.yuque.com/hollis666/asgng6/ivczis4gyskog9q2](https://www.yuque.com/hollis666/asgng6/ivczis4gyskog9q2) 就可以了）
...
SAX（**S**imple **A**PI for **X**ML）是一种基于**事件驱动**的XML解析模型，与DOM（Document Object Model） 这种将整个 XML 加载进内存构建树状结构完全不同。它的核心设计理念是**逐行边读边处理**，**不保留整个文档结构，**是一种**流式处理**方式。
